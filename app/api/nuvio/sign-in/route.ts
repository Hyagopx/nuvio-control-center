...
import { isSameOriginRequest, setRefreshCookie } from '../../../../lib/browser-session'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { email, password, remember = true } = await readJsonLimited(req, 16_000)
    if (typeof email !== 'string' || email.length > 320 || typeof password !== 'string' || password.length < 1 || password.length > 1_024) return NextResponse.json({ error: 'Informe um email e uma senha válidos.' }, { status: 400 })
    ...
    if (typeof d?.access_token !== 'string' || !d.access_token || typeof d?.refresh_token !== 'string' || !d.refresh_token) return NextResponse.json({ error: 'A Cloud API retornou uma sessão incompleta.' }, { status: 502, headers: { 'Cache-Control': 'no-store' } })
    const response = NextResponse.json({ access_token: d.access_token, user: d.user }, { headers: { 'Cache-Control': 'no-store' } })
    if (typeof d.refresh_token === 'string' && d.refresh_token) setRefreshCookie(response, d.refresh_token, remember === true)
    return response
  } catch (error:any) {
