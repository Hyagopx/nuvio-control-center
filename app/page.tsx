...
function AddonPage({profile,rows,diag,loading,diagnose,q,setQ,selected,setSelected,update,onAdd,onEdit,onCopy,token,lastSync}:any){
 ...
 return <div className="stack">
  ...
  {healthRows.length>0&&<div className="diag-strip"><span><i className="green-dot"></i>{healthy} saudáveis</span><span><i className="amber-dot"></i>{attention} atenção</span><span><i className="red-dot"></i>{failedCount} manifesto indisponível</span><span>{unverifiedCount} não verificáveis</span><span className="muted">Ativo indica configuração da conta; diagnóstico mede disponibilidade.</span></div>}
  <div className="panel table-panel"><div className="table-scroll mobile-card-table addon-card-table"><table><thead><tr><th>Addon</th><th>Catálogos</th><th>Estado</th><th>Diagnóstico</th><th>Ordem</th><th></th></tr></thead><tbody>{filtered.map((x:Addon,i:number)=>{const d=diag[x.url||''];const c=Array.isArray(d?.manifest?.catalogs)?d.manifest.catalogs.length:0;const order=x.sort_order??i;return <tr key={x.id||x.url||i} onClick={()=>setSelected(x)}>
...
function HealthCell({d}:{d?:Health}){if(!d)return <span className="muted">Não testado</span>;if(d.phase==='manifest'||d.phase==='catalog')return <span className="attention">◌ Verificando… <small>{d.summary?.tested||0}/{d.summary?.catalogs||0} catálogos</small></span>;const label=d.health==='healthy'?'Manifesto OK':d.health==='attention'?'Atenção':d.health==='fail'?'Manifesto indisponível':'Não verificável';const reason=d.healthReason||d.error;const summary=d.summary;const color=d.health==='fail'?'bad':d.health==='attention'||d.health==='unknown'?'attention':'ok';return <span title={reason||''} className={color}>{d.health==='healthy'?'✓':d.health==='attention'?'!':d.health==='fail'?'✕':'?'} {label} <small>{d.latencyMs?`${d.latencyMs}ms`:''}</small>{reason&&<small className="health-reason">{reason}</small>}{summary&&<small className="health-breakdown">{summary.tested||0}/{summary.catalogs||0} testados · {summary.failed||0} falhas · {summary.unverified||0} não verificáveis{summary.notTestedByLimit?` · ${summary.notTestedByLimit} sem teste pelo limite`:''}</small>}</span>}
