...
function moveRowTo<T>(rows:T[],from:number,to:number):T[]{const next=[...rows];if(from<0||from>=next.length)return next;const [item]=next.splice(from,1);next.splice(Math.max(0,Math.min(to,next.length)),0,item);return next}function AddonPage({profile,rows,diag,loading,diagnose,q,setQ,selected,setSelected,update,onAdd,onEdit,onCopy,token,lastSync}:any){const [failed,setFailed]=useState(false);const filtered=useMemo(()=>rows.filter((x:Addon)=>{const d=diag[x.url||''];return JSON.stringify(x).toLowerCase().includes(q.toLowerCase())&&(!failed||d?.health==='fail')}),[rows,diag,q,failed]);const healthRows=Object.values(diag) as Health[];const healthy=healthRows.filter(x=>x.health==='healthy').length,attention=healthRows.filter(x=>x.health==='attention').length,failedCount=healthRows.filter(x=>x.health==='fail').length,unverifiedCount=healthRows.filter(x=>x.health==='unknown'&&x.phase==='complete').length;function remove(item:Addon){if(!confirm(`Remover o addon "${item.name||item.display_name||item.url}"?`))return;update(rows.filter((x:Addon)=>!sameAddon(x,item)).map((x:Addon,i:number)=>({...x,sort_order:i})))}function move(item:Addon,to:number){const i=rows.findIndex((x:Addon)=>sameAddon(x,item));update(moveRowTo<Addon>(rows,i,to).map((x:Addon,n)=>({...x,sort_order:n})))}return <div className="stack"><div className="toolbar"><div className="search-wrap">⌕<input placeholder="Buscar addon…" value={q} onChange={e=>setQ(e.target.value)}/></div><button className="ghost" onClick={()=>setFailed(!failed)}>{failed?'Mostrar todos':'Somente falhos'}</button><button className="ghost" onClick={onAdd} disabled={!token}>Adicionar</button><button className="primary" disabled={loading} onClick={diagnose}>{loading?'Diagnosticando…':'Diagnosticar addons'}</button></div>{loading&&<div className="diag-strip"><span className="amber-dot"></span>{healthRows.length} de {rows.filter((x:Addon)=>x.url).length} addons responderam; diagnósticos parciais já estão disponíveis.</div>}{healthRows.length>0&&<div className="diag-strip"><span><i className="green-dot"></i>{healthy} saudáveis</span><span><i className="amber-dot"></i>{attention} atenção</span><span><i className="red-dot"></i>{failedCount} falhas</span><span>{unverifiedCount} não verificáveis</span><span className="muted">disponibilidade verificada separada dos alertas</span></div>}<div className="panel table-panel"><div className="table-scroll mobile-card-table addon-card-table"><table><thead><tr><th>Addon</th><th>Catálogos</th><th>Estado</th><th>Diagnóstico</th><th>Ordem</th><th></th></tr></thead><tbody>{filtered.map((x:Addon,i:number)=>{const d=diag[x.url||''];const c=Array.isArray(d?.manifest?.catalogs)?d.manifest.catalogs.length:0;return <tr key={x.id||x.url||i} onClick={()=>setSelected(x)}><td data-label="Addon"><div className="item-name">{x.name||x.display_name||d?.manifest?.name||'Sem nome'}</div><div className="item-sub">{x.url||'URL não informada'}</div></td><td data-label="Catálogos"><span className="number-chip">{c||'—'}</span></td><td data-label="Estado"><span className={'status-text '+(x.enabled===false?'off':'on')}><i></i>{x.enabled===false?'Desativado':'Ativo'}</span></td><td data-label="Diagnóstico"><HealthCell d={d}/></td><td data-label="Ordem"><OrderInput value={x.sort_order??i} max={rows.length} disabled={!token} onCommit={(to)=>move(x,to)}/></td><td data-label="Ações"><div className="row-actions"><button className="row-action" onClick={e=>{e.stopPropagation();onEdit(x)}}>Editar</button><button className="row-action danger" onClick={e=>{e.stopPropagation();remove(x)}}>Remover</button></div></td></tr>})}</tbody></table></div>{!filtered.length&&<div className="empty">Nenhum addon encontrado.</div>}</div>{selected&&<AddonDrawer item={selected} diag={diag[selected.url||'']} close={()=>setSelected(null)} update={update} rows={rows} token={token} profile={profile} lastSync={lastSync} onCopy={onCopy}/>}</div>}
function HealthCell({d}:{d?:Health}){if(!d)return <span className="muted">Não testado</span>;if(d.phase==='manifest'||d.phase==='catalog')return <span className="attention">◌ Verificando catálogos… <small>{d.summary?.tested||0}/{d.summary?.catalogs||0}</small></span>;const label=d.health==='healthy'?'Disponível':d.health==='attention'?'Atenção':d.health==='fail'?'Manifesto indisponível':'Não verificável';const reason=d.healthReason||d.error;return <span title={reason||''} className={d.health==='fail'?'bad':d.health==='attention'||d.health==='unknown'?'attention':'ok'}>{d.health==='healthy'?'✓':d.health==='attention'?'!':d.health==='fail'?'✕':'?'} {label} <small>{d.latencyMs?`${d.latencyMs}ms`:''}</small>{reason&&<small className="health-reason"> · {reason}</small>}</span>}
