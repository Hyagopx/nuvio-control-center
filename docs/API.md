...
| `/api/nuvio/diagnose` | POST | Lê manifestos de addons via SSE ou JSON (`stream: false`). Com `action: "check-catalogs"`, verifica apenas os `catalogIds` escolhidos pelo usuário. |
